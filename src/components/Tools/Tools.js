...
export default function Toolbox({ editorRef, textareaRef }) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("keydown", saveKeyDown);
  }, []);

  function removeParentIfHeadingOrParagraph(element) {
    while (
      element?.parentNode &&
      ["H1", "H2", "H3", "P"].includes(element.parentNode.tagName)
    ) {
  ...
  const getContent = async () => {
    ...
    try {
      const response = await fetch(
        "https://thoughtsdashboard.pythonanywhere.com/get_content"
      );
  ...
  const saveContent = async () => {
    ...
    try {
      const response = await fetch(
        "https://thoughtsdashboard.pythonanywhere.com/update_content",
        {
          method: "POST",
          headers: {
